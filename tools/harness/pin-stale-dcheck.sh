...
# Netz (GitHub-Releases-API + Registry); meldet, hebt nicht an. Die
# API-Abfrage (Achse B) laeuft containerisiert ueber
...
# dem gepinnten Tag.
latest=$(github_api_get "repos/pt9912/d-check/releases/latest" | grep -m1 '"tag_name"' | sed -E 's/.*"tag_name":[[:space:]]*"([^"]+)".*/\1/')
if [ -z "$latest" ]; then

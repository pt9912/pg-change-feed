...
SDK_INTEGRATION_IMAGE=${SDK_KOTLIN_INTEGRATION_IMAGE:-pg-change-feed:sdk-kotlin-integration}
SDK_TEST_CONTAINER=cdc-sdk-kotlin-client-test

TEST_TABLE=feed_e2e_full
ABDECKUNG_ZIEL_DATEI=docs/user/sdk-e2e-abdeckung.md
GRPC_TEST_NAME=GrpcRealserverTest
SSE_TEST_NAME=SseRealserverTest
NATS_TEST_NAME=NatsRealserverTest
HTTP_TEST_NAME=HttpRealserverTest
GRPC_SENTINEL=KotlinGrpcSdkE2ESentinel
SSE_SENTINEL=KotlinSseSdkE2ESentinel
NATS_SENTINEL=KotlinNatsSdkE2ESentinel
...
if [ "$ready" -ne 1 ]; then
  echo "run-sdk-kotlin-integration-tests: Compose-PostgreSQL wurde nicht bereit" >&2
  exit 1
fi
...
if [ "$healthy" -ne 1 ]; then
  echo "run-sdk-kotlin-integration-tests: Feed-Container meldet Compose-Health-Status ${health:-fehlt}, wollen healthy" >&2
  exit 1
fi

docker build --build-context proto=proto -f sdks/kotlin/Dockerfile \
...
run_phase() {
  ...
  if [ "$test_ready" -ne 1 ]; then
    echo "run-sdk-kotlin-integration-tests: $phase_name — Test wurde nicht innerhalb der Zeitspanne bereit (kein READY): $(docker logs "$SDK_TEST_CONTAINER" 2>&1 || true)" >&2
    exit 1
  fi
...
SQL
  ...
  if [ "$test_stopped" -ne 1 ] || [ "$test_exit" != "0" ]; then
    echo "run-sdk-kotlin-integration-tests: $phase_name — der Test endete nicht mit Ausgang 0 (gestoppt: $test_stopped, Ausgang: $test_exit): $test_output" >&2
    exit 1
  fi
  ...
  if [ "$feed_running" != "true" ]; then
    echo "run-sdk-kotlin-integration-tests: $phase_name — Feed-Container lief nach dem Lauf nicht mehr weiter (kein Neustart erwartet)" >&2
    exit 1
  fi
...
abdeckung_schreiben() {
  ...
  if [ -f "$ABDECKUNG_ZIEL_DATEI" ]; then
    if grep -q "pgchangefeed-sdk-e2e:kotlin-begin" "$ABDECKUNG_ZIEL_DATEI"; then
      vor=$(awk '/pgchangefeed-sdk-e2e:kotlin-begin/{ausgang=1} !ausgang{print}' "$ABDECKUNG_ZIEL_DATEI")
      nach=$(awk 'f{print} /pgchangefeed-sdk-e2e:kotlin-end/{f=1}' "$ABDECKUNG_ZIEL_DATEI")
    else

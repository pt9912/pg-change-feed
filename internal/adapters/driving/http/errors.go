...
// — einheitlich über alle sechs Fähigkeiten dieses Slice hinweg
// (`slice-060` §1 Ziel): `inbound.ErrSourceTableMissing` trägt eine
// unbekannte Ressource an der Quelle (`404`); die benannten
// Domänen-Invarianten (`ADR-0029`) tragen eine ungültige Eingabe (`400`) —
// dieselbe Klasse wie `domainerrors.ErrEmptyIdentifier` in
// `registerconsumer.go`; jeder übrige (unbekannte) Fehler ist ein
// unerwarteter interner Fehler (`500`) und wird geloggt, nicht nur
// verworfen.

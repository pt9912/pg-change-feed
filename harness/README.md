...
| `make docs-check` | kaputte Referenzen in der Markdown-Doku (links, anchors, ids, matrix, versions, structure, hostpaths) | [`harness/sensors/docs-check.md`](sensors/docs-check.md) |
...
| `make examples-csharp` | baut die drei Werkzeugketten-Images der C#-Sprach-Wurzel (Bau-Kontext `examples/csharp/`, eigenes digest-gepinntes Dockerfile, gemeinsame `build`-Stufe) — `pg-change-feed-examples:csharp` (http-client), `pg-change-feed-examples:csharp-sse` (sse-client) und `pg-change-feed-examples:csharp-nats` (nats-client) — und fährt darin `dotnet restore`/`build`/`test` für alle drei Programme; ein roter Test bricht den `docker build` ab; die Beispiele bleiben Doku mit Bau-Bindung, kein Lauf-Beleg. Braucht Netz (NuGet-Paketbezug), deshalb Werkzeug statt Gate | kein Gate, [`ADR-0087`](../docs/plan/adr/0087-beispiel-clients-csharp-kotlin.md), [`ADR-0090`](../docs/plan/adr/0090-beispiel-clients-volle-matrix.md) · seit slice-098, erweitert seit slice-100, seit slice-101 |
| `make examples-kotlin` | baut die drei Werkzeugketten-Images der Kotlin-Sprach-Wurzel (Bau-Kontext `examples/kotlin/`, eigenes digest-gepinntes Dockerfile, gemeinsame `build`-Stufe) — `pg-change-feed-examples:kotlin` (http-client), `pg-change-feed-examples:kotlin-sse` (sse-client) und `pg-change-feed-examples:kotlin-nats` (nats-client) — und fährt darin den Gradle-Wrapper (`test`/`installDist`) für alle drei Module; ein roter Test bricht den `docker build` ab; die Beispiele bleiben Doku mit Bau-Bindung, kein Lauf-Beleg. Braucht Netz (Maven-Central-/Gradle-Plugin-Portal-Paketbezug), deshalb Werkzeug statt Gate | kein Gate, [`ADR-0087`](../docs/plan/adr/0087-beispiel-clients-csharp-kotlin.md), [`ADR-0090`](../docs/plan/adr/0090-beispiel-clients-volle-matrix.md) · seit slice-099, erweitert seit slice-100, seit slice-101 |

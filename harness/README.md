...
| `make doc-trace` | gibt die Requirements Traceability Matrix (RTM) auf stdout aus (`--trace`, **kein** eigenes Modul, advisory — Exit 0 auch mit Requirements-Waisen); `trace.requirements.id-pattern` in `.d-check.yml` verdrahtet die RTM-Quelle auf die `LH-(FA|QA)-[A-Z]{3}-\d{3}`-Kennungskonvention dieses Repos statt d-checks generischen Default, `trace.coverage` liest zusätzlich `docs/user/e2e-abdeckung.md`, `docs/user/bench-abdeckung.md` und `docs/user/ci-matrix-abdeckung.md` als kuratierte Coverage-Dimensionen, damit eine Anforderung mit ausschließlichem E2E-, Bench- oder CI-Matrix-Nachweis nicht fälschlich als Waise erscheint (real gemessen nach dem Tag-Nachtrag für die letzten zwölf RTM-Waisen (`LH-FA-CON-001`…`005`, `LH-FA-DAT-002`/`003`/`005`, `LH-FA-REA-001`, `LH-FA-RET-001`, `LH-QA-REL-003`/`004`) an bereits bestehenden E2E-Belegen: 76 Anforderungen, 55 Waisen ohne `trace.coverage`, **0 Waisen mit** — jede Anforderung trägt mindestens einen Beleg). `make doc-complete`/`--require-complete` bleibt trotzdem bewusst **nicht** in `GATE_CHECKS`/`make gates`: die 0-Waisen-Zahl ist ein realer Momentaufnahme-Stand, kein struktureller Ausschluss künftiger Waisen — eine neue Anforderung ohne Beleg wäre erst beim nächsten `make doc-trace`-Lauf sichtbar, nicht automatisch gate-blockierend, solange `make doc-complete` nicht Teil der Closure ist | kein Gate, wie `make image-stale`, [`harness/sensors/docs-check.md`](sensors/docs-check.md) §Grenze · seit slice-d-check-trace-rtm |
...
| `.github/workflows/release.yml` | Release-Workflow (`ADR-0051` Entscheidung 3), Trigger ausschließlich `push: tags: ['v*']` (`ci.yml`/`e2e.yml` schließen Tag-Pushes per `tags-ignore` aus, kein Doppellauf): validiert den Tag strikt gegen SemVer 2.0, gleicht ihn gegen `docs/user/version.md` am getaggten Commit ab (Abbruch bei Abweichung, vor jedem Login/Build/Push), baut über `make image VERSION=<Version>` (ein Build), pusht nach GHCR **und** Docker Hub (Content-Mirror), setzt `:latest` auf beiden Registries nur bei einem stabilen (nicht-Prerelease) Tag (`LATEST=true`), legt abschließend über die auf dem Runner vorinstallierte `gh`-CLI ein GitHub-Release mit dem Image-Digest im Beschreibungstext an | kein Gate, [`ADR-0051`](../docs/plan/adr/0051-cicd-pipeline-github-actions.md) |

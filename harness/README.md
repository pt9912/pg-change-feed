...
| `make examples-csharp` | baut das Werkzeugketten-Image der C#-Sprach-Wurzel (Bau-Kontext `examples/csharp/`, eigenes digest-gepinntes Dockerfile) und fährt darin `dotnet restore`/`build`/`test` — ein roter Test bricht den `docker build` ab; die Beispiele bleiben Doku mit Bau-Bindung, kein Lauf-Beleg. Braucht Netz (NuGet-Paketbezug), deshalb Werkzeug statt Gate | kein Gate, [`ADR-0087`](../docs/plan/adr/0087-beispiel-clients-csharp-kotlin.md), [`ADR-0090`](../docs/plan/adr/0090-beispiel-clients-volle-matrix.md) · seit slice-098 |
| `make examples-kotlin` | baut das Werkzeugketten-Image der Kotlin-Sprach-Wurzel (Bau-Kontext `examples/kotlin/`, eigenes digest-gepinntes Dockerfile) und fährt darin den Gradle-Wrapper (`test`/`installDist`) — ein roter Test bricht den `docker build` ab; die Beispiele bleiben Doku mit Bau-Bindung, kein Lauf-Beleg. Braucht Netz (Maven-Central-/Gradle-Plugin-Portal-Paketbezug), deshalb Werkzeug statt Gate | kein Gate, [`ADR-0087`](../docs/plan/adr/0087-beispiel-clients-csharp-kotlin.md), [`ADR-0090`](../docs/plan/adr/0090-beispiel-clients-volle-matrix.md) · seit slice-099 |

...
| `make doc-trace` | gibt die Requirements Traceability Matrix (RTM) auf stdout aus (`--trace`, **kein** eigenes Modul, advisory — Exit 0 auch mit Requirements-Waisen); `trace.requirements.id-pattern` in `.d-check.yml` verdrahtet die RTM-Quelle auf die `LH-(FA|QA)-[A-Z]{3}-\d{3}`-Kennungskonvention dieses Repos statt d-checks generischen Default, `trace.coverage` liest zusätzlich `docs/user/e2e-abdeckung.md`, `docs/user/bench-abdeckung.md` und `docs/user/ci-matrix-abdeckung.md` als kuratierte Coverage-Dimensionen, damit eine Anforderung mit ausschließlichem E2E-, Bench- oder CI-Matrix-Nachweis nicht fälschlich als Waise erscheint (real gemessen nach `LH-FA-SST-001`/`005`, `LH-FA-CFG-006`, `LH-QA-PER-004`, `LH-QA-POR-001`/`002` (`ADR-0105`): 76 Anforderungen, 55 Waisen ohne `trace.coverage`, 12 Waisen mit). `make doc-complete`/`--require-complete` bleibt bewusst **nicht** in `GATE_CHECKS`/`make gates` — die verbleibenden 12 Waisen (`LH-FA-CON-001`…`005`, `LH-FA-DAT-002`/`003`/`005`, `LH-FA-REA-001`, `LH-FA-RET-001`, `LH-QA-REL-003`/`004`) sind real bereits über bestehende E2E-Phasen mitbelegt, nur ohne eigenen Kennungs-Tag am jeweiligen Testfall — eine unsanierte Bestandslücke, deren Sanierung ein eigener inhaltlicher Vorgang wäre, kein Konfigurations-Vorgang | kein Gate, wie `make image-stale`, [`harness/sensors/docs-check.md`](sensors/docs-check.md) §Grenze · seit slice-d-check-trace-rtm |

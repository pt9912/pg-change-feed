...
| `make doc-trace` | gibt die Requirements Traceability Matrix (RTM) auf stdout aus (`--trace`, **kein** eigenes Modul, advisory — Exit 0 auch mit Requirements-Waisen); `trace.requirements.id-pattern` in `.d-check.yml` verdrahtet die RTM-Quelle auf die `LH-(FA|QA)-[A-Z]{3}-\d{3}`-Kennungskonvention dieses Repos statt d-checks generischen Default, `trace.coverage` liest zusätzlich `docs/user/e2e-abdeckung.md` und `docs/user/bench-abdeckung.md` als kuratierte Coverage-Dimensionen, damit eine Anforderung mit ausschließlichem E2E- oder Bench-Nachweis nicht fälschlich als Waise erscheint (real gemessen nach `LH-QA-PER-001`/`002`/`003` (`ADR-0104`): 76 Anforderungen, 55 Waisen ohne `trace.coverage`, 18 Waisen mit). `make doc-complete`/`--require-complete` bleibt bewusst **nicht** in `GATE_CHECKS`/`make gates` — die verbleibenden 18 Waisen zerfallen in zwei Klassen: sechs tragen ihren Beleg real auf einer anderen Ebene als `test-integration` (`LH-FA-SST-001`/`005`, `LH-FA-CFG-006` — architektonische Aussagen, durch das ganze System bewiesen; `LH-QA-PER-004` — `make bench`, [`ADR-0054`](../docs/plan/adr/0054-coverage-gate-und-benchmark-infrastruktur.md) §(b) entscheidet bewusst gegen ein E2E-Gate; `LH-QA-POR-001`/`002` — CI-Matrix in `.github/workflows/e2e.yml`, nicht `test-integration`) und bräuchten keinen neuen Test, sondern höchstens eine erweiterte Coverage-Dimension für diese Ebenen; die restlichen zwölf (`LH-FA-CON-001`…`005`, `LH-FA-DAT-002`/`003`/`005`, `LH-FA-REA-001`, `LH-FA-RET-001`, `LH-QA-REL-003`/`004`) sind real bereits über bestehende E2E-Phasen mitbelegt, nur ohne eigenen Kennungs-Tag am jeweiligen Testfall — beides sind unsanierte Bestandslücken, deren Sanierung ein eigener inhaltlicher Vorgang wäre, kein Konfigurations-Vorgang | kein Gate, wie `make image-stale`, [`harness/sensors/docs-check.md`](sensors/docs-check.md) §Grenze · seit slice-d-check-trace-rtm |

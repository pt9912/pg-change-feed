...
**Source & issues:** [github.com/pt9912/pg-change-feed](https://github.com/pt9912/pg-change-feed)
...
| **Distribution** | Available as an OCI image for `linux/amd64` **and** `linux/arm64` (GHCR and Docker Hub, identical digest); the HTTP API and gRPC stream are also available as an official C# client library ([`PgChangeFeed.Client`](https://www.nuget.org/packages/PgChangeFeed.Client) on NuGet.org), and the HTTP API as an official Python client library ([`pgchangefeed`](https://pypi.org/project/pgchangefeed/) on PyPI). |

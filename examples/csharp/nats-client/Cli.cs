...
/// <c>examples/nats-client/main.go</c> (<c>parseFlags</c>).
/// </summary>
    ...
    public static Config Parse(string[] args, Func<string, string?> getEnv)
    {
        ...
        for (var i = 0; i < args.Length; i++)
        {
            ...
            string NextValue()
            {
                ...
                if (i + 1 >= args.Length)
                {
                    throw new ArgumentException($"nats-client: Flag {name} braucht einen Wert");
                }
                return args[++i];
            }
            ...
            switch (name)
            {
                ...
                case "--table":
                    ...
                    break;
                default:
                    throw new ArgumentException($"nats-client: unbekanntes Flag {name}");
            }
        }

        return new Config(natsUrl, addr, token, source, schema, table);
    }

...
|---|---|---|---|
| Aufrufer der gemeinsamen Funktion | `grep -rn` nach dem Funktionsnamen der gemeinsamen Funktion (am Start gemessen) über `internal --include=*.go` | *(Implementer trägt ein)* | jeden Aufrufer nachziehen; der Backfill-Pfad übergibt die leere Menge, mit Kommentar-Bezug auf `backfill-pfad` |

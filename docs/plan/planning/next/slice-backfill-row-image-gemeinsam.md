...
|---|---|---|
| `internal/domain/model/rowimage.go` (Arbeitsname) | neu | die eine Funktion; reine Funktion, keine geteilten Zustände; Eingabe neutral. |
| `internal/domain/model/rowimage_test.go` | neu | Referenz-Bytes vom Parent-Stand, Randfälle, Determinismus (`-race`). |
| `internal/adapters/driving/replication/mapper/mapper.go` | update | `Assembler.change` ruft die Domänen-Funktion; `rowImage` entfällt; `containsColumn` bleibt, soweit `appendExcluded`/`removeExcluded` es brauchen. |
...
|---|---|---|---|
| Go-Code und Kommentare | `grep -rn 'rowImage' --include=*.go .` | *(Implementer trägt ein)* | nachziehen |
| Dokumente (Spec, Handbuch, Harness) | `grep -rn 'rowImage' --include=*.md .` | *(Implementer trägt ein)* | aktive Träger nachziehen |
| `Accepted` ADRs, die den Ort als Kontext nennen (`mapper.rowImage`) | dieselbe Suche, Treffer unter `docs/plan/adr/` | *(Implementer trägt ein)* | nicht ändern: der Ort steht dort als Stand zum Entscheidungszeitpunkt; nur eine reine Verweisgerüst-Korrektur wäre nach [`ADR-0073`](../../adr/0073-zitat-korrektur-an-immutablen-dokumenten.md) zulässig ([`AGENTS.md`](../../../../AGENTS.md) §3.5) |
| Zweite JSON-Erzeugung für Bilder | `grep -rn 'json.Marshal' internal --include=*.go` (Fundstellen nach Bild-Bezug lesen) | *(Implementer trägt ein)* | Fundstellen, die ein Row Image bauen, gehören in die eine Funktion |

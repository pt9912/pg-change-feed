...
benennen**, färbten nicht rot — gefunden vom Review zu `slice-091` (F-1, F-2)
...
aus `slice-091` selbst (sie sind Teil dieses Diff), gefunden wurden sie im
Review und im Delta-Review zu `slice-091`; die zitierte Zählung ist
...
Quelle: Review zu `slice-091` (F-1, F-2) ·
Delta-Review zu `slice-091` (D-1) ·

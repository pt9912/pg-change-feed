...
`slice-backfill-run-store`; der Beleg im Adapter steht dort: `TestBackfillRunFinishOnAnEndedRunIsANoOpSuccess`
(Endzustand wirkungslos, Mutation `Finish`-`WHERE` erweitert → rot) und
`TestBackfillRunOperationsEndAtTheirOwnTimeoutOnDetachedContext` samt
`TestBackfillAdmissionAndWriterEndAtTheirOwnTimeoutOnDetachedContext` (Zeitbegrenzung bei
abgelöstem Kontext, Mutation `boundedContext` entfernt → rot; Verifikation §4, M1 und M4).
Bei 3× wäre zu

...
Tests blieb bei fehlendem `search_path` grün, weil `NOT (… AND NULL)` NULL ist; Ausprägung: die
Prüfung einer Katalog-Spalte trägt den NULL-Fall (`IS DISTINCT FROM`). Neue Form (anderer
Träger-Typ), deshalb Datei trotz Deckel. Der fünfzehnte Beleg

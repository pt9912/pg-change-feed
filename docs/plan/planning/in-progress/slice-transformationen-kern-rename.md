...
| `internal/domain/model/rowimage.go` (`BuildRowImage`, die gemeinsame Row-Image-Funktion aus `slice-backfill-row-image-gemeinsam`) | update | nimmt den Regelsatz als vierten Parameter `rules []Transformation`, wertet ihn nach dem Ausschluss in derselben Schleife aus und schreibt nie zwei gleichnamige Schlüssel (Kollision: `ErrTransformationTargetCollides`, kein Bild); alle Aufrufer nachgezogen (Suchlauf). |
| `internal/adapters/driving/replication/mapper/mapper.go` | update | `TableBinding.Transformations`, `Assembler.SetTransformation`/`RemoveTransformation`, Erhalt bei `AddBinding`-Merge und `setSchemaVersion`, Anwendbarkeits-Prüfung in `change`, `ErrTransformationNotApplicable`, `imageError` (ordnet den Kollisions-Fehler der Row-Image-Konstruktion als nicht anwendbare Regel ein). |
...
| `internal/domain/model/transformation_test.go` | neu | Konstruktor-Invarianten je Eingabe, Regeltyp-Menge, `CheckApplicable`, Wirkung in `BuildRowImage` (Position, Abwesenheit, Ausschluss, Maskierung, leere Regelmenge, Reinheit unter `-race`, keine zwei gleichnamigen Schlüssel). |
| `internal/adapters/driving/replication/mapper/transformation_test.go`, `transformation_internal_test.go`, `mapper_bench_test.go` | neu | die Mapper-Tests liegen in eigenen Dateien statt in `mapper_test.go` (dort unverändert): Wirkung auf beide Images, Metadaten, Nichtanwendbarkeit an jeder Position der Regelliste, Kollision zweier Regeln mit gleichem Zielnamen, Live-Reload, Erhalt, Fitness-Function-Tests; der Whitebox-Test hält die Schnappschuss-Zusage (Liste wird neu aufgebaut); der Benchmark misst die Kosten der Prüfung. |
...
- **`BuildRowImage` prüft die Anwendbarkeit nicht selbst, schreibt aber nie zwei gleichnamige
  Schlüssel.** Die Anwendbarkeit ist Vorbedingung des Aufrufers (Doc-Kommentar); der Mapper prüft je
  Änderung vor jeder Serialisierung. Die Funktion trägt eine billige Kollisionsprüfung an der Stelle,
  an der sie den Zielschlüssel schreibt: gleicht der Zielname einer Spalte aus `columns` oder dem
  Zielnamen einer anderen im Bild umbenannten Spalte, liefert sie `ErrTransformationTargetCollides`
  und kein Bild; der Mapper ordnet den Fehler über `imageError` als
  `ErrTransformationNotApplicable` ein. Damit steht die Zusage des Doc-Kommentars im Code (K3 zwischen
  Regeln bleibt Prüfung des Use Case beim Antrag, die Funktion ist der zweite Wächter). *Beleg-Anker:*
  Kollisions-Probe des Reviews (`docs/reviews/review-slice-transformationen-kern-rename.md` F-1/F-5),
  Tests `TestBuildRowImageNeverWritesTwoEqualKeys` und
  `TestConsumeTwoRulesWithSameTargetAreNotApplicableWhenBothMatch`; `BenchmarkAssemblerChange…` nach
  der Prüfung unverändert 2444 B/op, 86 allocs/op (Lauf in den Belegen). Gewählt statt der reinen
  Umformulierung des Kommentars, weil die Prüfung ohne Signaturbruch in wenigen Zeilen an der
  Schreibstelle sitzt.
...
diff 55 -n BuildRowImage -- internal/*.go
...
- Sensoren: `make test` (Race-Detector) Exit 0, 42 Pakete `ok`; `make a-check` Exit 0, `gesamt: 0 Befund(e)`; `make coverage-gate` Exit 0, `coverage-gate: OK — Coverage 83.90% erfüllt Schwelle 80%` (Domäne und `replication/mapper` liegen in der gemessenen Fläche, kein neues Paket); `make gates` ungefiltert in ein Log geschrieben, Exit-Code gesondert gelesen: Exit 0 (`coverage-gate: OK — Coverage 83.90% erfüllt Schwelle 80%`, `d-check: 1209 Datei(en) geprüft, 0 Befund(e)`, `commit-traceability: OK — 5 Commit(s)`, `generated-sync: OK`, `gesamt: 0 Befund(e)`). Alle Messungen dieses Punkts stehen am Endstand des Codes (Arbeitsbaum mit den Änderungen der Review-Fixrunde). `make suchlauf-nachmessen PLAN=<dieser Plan>` Exit 0, 14 Zeilen stimmen (nach den Änderungen der Fixrunde nachgemessen, die Zahl der Zeile „Aufrufer der gemeinsamen Funktion“ am Diff-Stand nachgezogen).
...
- Zusage · mutierte Eingabe · gesehenes Rot (alle Mutationen einzeln gefahren, danach per `cmp` gegen die Sicherungskopie zurückgenommen):
  ...
  - Keine zwei gleichnamigen Schlüssel · (a) Prüfung gegen `columns` aus `BuildRowImage` entfernt, (b) Prüfung gegen die umbenannten Schlüssel entfernt, (c) `imageError` bei einem bzw. beiden `BuildRowImage`-Aufrufen im Mapper ausgelassen · (a) `TestBuildRowImageNeverWritesTwoEqualKeys` (vier Spaltenfälle), (b) derselbe Test (Fall „zwei Regeln“) und `TestConsumeTwoRulesWithSameTargetAreNotApplicableWhenBothMatch` (Neu- und Alt-Image), (c) derselbe Mapper-Test (Alt-Image bzw. beide).
  - Jede Regel der Liste geprüft · `checkTransformations` über `rules[:1]` bzw. `rules[1:]` bzw. `rules[:len(rules)-1]` · `TestConsumeEveryRuleIsCheckedForApplicability` (drei Fälle: zweite, letzte, mittlere Regel; ein Fall: erste Regel; zwei Fälle: zweite und letzte).
...
- **Randfälle ohne eigene Bindung oder mit geteiltem Speicher (benannt).**
  ...
  unverändert“ steht im Doc-Kommentar von `TableBinding`, eine Kopie beim
  Eintritt ist nicht gefahren. (b) Sind Spalte und Zielname einer Regel
  zugleich verletzt (Spalte fehlt, Zielname kollidiert), bestimmt die
  Prüfreihenfolge in `CheckApplicable` den Sentinel; kein Test bindet sie.

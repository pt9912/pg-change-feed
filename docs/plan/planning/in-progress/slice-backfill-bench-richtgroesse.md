...
- [ ] Bench: `tools/bench-backfill.sh` (Arbeitsname) läuft in `make bench` mit
      ...
      abgeleitet. *Offen:* `make bench` endet mit Exit 2 am vorhandenen Skript
      `tools/bench-source-impact.sh` (`LH-QA-PER-001`, Overhead 87,5 % und 93,9 %
      gegen die 35-%-Schwelle in zwei Läufen; derselbe Befund am Parent
      `933ab054` mit 95,8 %) und erreicht das neue Skript nicht; das neue Skript
      läuft einzeln mit Exit 0.
...
| `internal/adapters/driven/postgresstorage/backfillstatusview_test.go` | update (Plan-Nachzug) | die View reicht `warn_estimated_size` durch: der Test prüft neben `warn_duration` auch diese Spalte; DoD-Beleg „View trägt das Ergebnis“ (`make test-store`). |
...
- **Die WAL-Fehlerschwelle kann vor der Richtgröße greifen.** Ein Run schreibt den Bestand in den CDC-Speicher derselben Datenbank; das WAL zählt zum Rückstand des Capture-Slots. Ein dritter Run über 1.000.000 Zeilen im selben Speicher überschritt die Fehlerschwelle von 1 GiB (`SPEC-013`): der Feed-Container endete mit Exit 1 (`replication`), der Run `interrupted`. Die Richtgröße (Rate mal Toleranz) kennt diese zweite Größe nicht; ob die Warnung (1) oder die Richtgröße sie berücksichtigt, ist eine Entscheidung (Folge-ADR), keine Umsetzung dieses Slice.
- **Die Speicher-Spitze des Feed-Containers liegt weit über `B` mal Zeilenbreite** (Handbuch: 467 MiB bei 200.000 Zeilen, 1.544 MiB bei 1.000.000 Zeilen gegen etwa 74 KB je Block); die Ursache ist nicht untersucht.
- **`make bench` bricht am ersten roten Skript ab**; `LH-QA-PER-001` ist in dieser Umgebung rot (auch am Parent), das neue Skript folgt danach und wird nicht erreicht.
- **Die Kopierrate streut zwischen Läufen um etwa den Faktor 2**; die Richtgröße hängt am Lauf, aus dem sie abgeleitet ist.

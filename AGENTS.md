...
### 3.14 Ein Aufrufer von `make schema-rollout` gegen ein möglicherweise bereits migriertes Ziel trägt seine eigene Idempotenz-Wache

`make schema-rollout` (d-migrate) ist nicht idempotent gegen ein bereits
migriertes Ziel, sobald dieses Ziel **Fremdobjekte** trägt — Datenbankobjekte
außerhalb des neutralen Modells (`tools/schema/schema.yaml`), angelegt über
eigene `tools/schema/nacharbeit-*.sql`-Skripte (aktueller Bestand: zwei Views
für Heartbeat/Observability, vier SQL-Funktionen für Tabellen-Aktivierung und
Spaltenausschluss). Ein zweiter Lauf gegen ein bereits migriertes Ziel sieht
d-migrate diese Objekte als nicht deklariert an und plant ihren Abbau — Exit 8
(`DESTRUCTIVE_OPERATION_REQUIRES_CONFIRMATION`), pin-unabhängig real
reproduziert. Der betroffene Objektumfang ist mit jedem neuen
`nacharbeit-*.sql`-Skript gewachsen (2 → 4 → 6, siehe
`docs/plan/planning/observations/BEO-PGC/schema-rollout-fremdobjekte/`) und
wächst mit jedem weiteren.

Der naheliegende Fix — die Fremdobjekte ins neutrale Modell überführen — ist
für die Views bereits gelungen (seit slice-016), bleibt für die SQL-Funktionen
aber blockiert: `schema migrate --execute` bricht für jede über den
`functions:`-Knoten deklarierte Funktion mit `POST_EXECUTE_DRIFT` (Exit 5) ab
— ein Upstream-Werkzeug-Verhalten, kein Repo-Fehler, real isoliert mit einer
trivialen No-Arg-Funktion reproduziert
(`docs/plan/planning/observations/BEO-PGC/d-migrate-nacharbeit/`). Solange das
nicht behoben ist, bleibt der Weg über das neutrale Modell für Funktionen eine
Sackgasse; ein generisches `--allow-destructive`-Handling im
`schema-rollout`-Target wäre der falsche Gegenzug, weil es echte destruktive
Änderungen ununterscheidbar von bekannten Fremdobjekten durchließe.

Jedes Skript oder Makefile-Target, das `make schema-rollout` gegen ein Ziel
aufruft, das bereits migriert gewesen sein könnte — Bootstrap-Skripte,
Beispiel-Umgebungen, jeder wiederholbare Automatisierungs-Lauf —, prüft daher
selbst vorab, ob das Ziel schon migriert ist, und überspringt den
Rollout-Aufruf in diesem Fall. Ein einfacher Existenz-Check gegen ein
bekanntes Kernobjekt reicht (`to_regclass('cdc.source_table')` oder
gleichwertig).

**Falsch:** ein neues Bootstrap-/Beispiel-/Automatisierungs-Skript ruft
`make schema-rollout` unbedingt auf, ohne zu prüfen, ob das Ziel schon
migriert ist — ein zweiter Lauf gegen dasselbe Ziel bricht real mit Exit 8 ab.
**Richtig:** vor dem Aufruf ein Existenz-Check auf ein bekanntes Kernobjekt
(Vorbild: `examples/bootstrap.sh`); `make schema-rollout` läuft nur bei
Nichtexistenz.

**Begründung:** Dreifach real und unabhängig aufgetreten
(`docs/plan/planning/observations/BEO-PGC/schema-rollout-fremdobjekte/`,
`evidence/slice-016.md`, `evidence/slice-063.md`,
`evidence/slice-beispiele-compose-bootstrap.md`) — jeder Aufrufer fand
denselben lokalen Workaround unabhängig neu, weil keine Stelle das Muster
vorab benannte. Der Root-Cause-Fix bleibt für die Funktions-Klasse an ein
Upstream-Verhalten von d-migrate gebunden
(`docs/plan/planning/observations/BEO-PGC/d-migrate-nacharbeit/`) und ist
damit kein Ein-Zeilen-Fix und derzeit nicht terminierbar; ein
`--allow-destructive`-Umgang im Makefile-Target wäre grobkörniger als das
Problem, das er löst. Der Existenz-Check ist demgegenüber billig (zwei bis
drei Zeilen) und dort verortet, wo die nötige Information — läuft dieser

...
This package is at an early, pre-1.0 stage (`0.x.y`, [ADR-0109](https://github.com/pt9912/pg-change-feed/blob/main/docs/plan/adr/0109-kotlin-github-packages-drittes-sdk-package.md)). This initial release provides only the shared connection configuration (`PgChangeFeedClientOptions`: server address and bearer token) — a full HTTP API client surface and the gRPC live-change-stream surface follow in subsequent releases (`ADR-0109` Festlegung 1). SSE and NATS-vollinhalt delivery remain out of scope for this package's planned first full release and would be added by a later follow-up (`ADR-0109` Festlegung 1).

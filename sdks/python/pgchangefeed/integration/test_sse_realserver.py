"""Realserver integration test for the SSE stream client surface (SPEC-021).
...
The commit-to-delivery window is fire-and-forget (SPEC-021): between the
stream opening and the server registering the subscriber at the Broadcaster,
a committed change can be discarded for this subscriber. The runner commits
...
def test_realserver_receives_a_committed_change_over_the_stream() -> None:
    ...
    # Feldvollstaendigkeit am realen Wire-Image (SPEC-021): alle zehn Felder
